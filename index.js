/*
 * WhyNotHomes code not contributed by third parties is licensed by Clayton Rothschild.
 * Marzipano is licensed by Copyright 2016 Google Inc. All rights reserved.
 *
...
'use strict';
  ...
  if (window.matchMedia) {
    ...
    portraitMediaQueryList.addListener(setMode);
...
function nextInArray() {
    if (sceneIndex != scenes.length-1){
      switchScene(scenes[sceneIndex + 1])
    }
  }
...
nextElement.addEventListener('click',  function() {
      nextInArray(viewer._scene)});

previousElement.addEventListener('click', function() {
      prevInArray(viewer._scene)})